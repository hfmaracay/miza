  ...
  <div class="col-md-10">
    <div class="card shadow mb-4">
      <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Agregar Noticia</h6>
      </div>
      <div class="card-body">
        ...
        @endif
        <form id="form-create" name="form-create" method="POST" action="{{ route('adminNews.store') }}" enctype="multipart/form-data" class="form-class needs-validation" novalidate>
          @csrf
          <div class="form-group">
            <label for="name" class="sr-only">Título</label>
            <input type="text" name="name" id="name" class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" value="{{ old('name') }}" placeholder="Título" required />
          ...
          <div class="form-group">
            ...
            <textarea name="description" id="description" cols="30" rows="5" class="form-control textareacontent {{ $errors->has('description') ? 'is-invalid' : '' }}" placeholder="Descripción" required>{{ old('description') }}</textarea>
            <div class="invalid-feedback">
          ...
          <div class="form-group files">
            ...
            <div class="invalid-feedback">
              {{ $errors->has('image') ? $errors->first('image') : 'Foto es Requerida' }}
            </div>
          </div>
